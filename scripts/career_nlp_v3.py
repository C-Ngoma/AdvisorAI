...
def match_jobs(query, top_n=5):
    ...
    if entities['skills']:
        results = results[results['required_skills'].apply(
            lambda x: any(skill.lower() in str(x).lower() for skill in entities['skills'])
        )]
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print(real_results[['job_title','company_name','salary_usd','industry','company_location']].to_string(index=False))
