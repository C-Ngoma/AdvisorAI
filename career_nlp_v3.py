...
# Prepare training data (using job title + skills for context)
X_text = (df['job_title'] + " " + df['required_skills'].fillna('')).tolist()
X = embed_model.encode(X_text, convert_to_tensor=False)  # Convert to numpy array
y = df['industry']
...
# Updated detect_intent function
def detect_intent(query, top_n=1):
    """
    Returns top_n predicted industries for a student query
    """
    ...
    if top_n == 1:
        ...
        top_industries = [clf.classes_[i] for i in top_indices]
